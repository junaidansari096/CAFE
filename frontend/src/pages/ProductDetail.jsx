...
export default function ProductDetail() {
  ...
  return (
    <div className={`min-h-screen pt-32 pb-20 px-6 md:px-20 transition-colors duration-700 ${isDark ? 'bg-[#0d0f0f] text-[#fafaf5]' : 'bg-[#fafaf5] text-zinc-900'}`}>
      <button onClick={() => navigate(-1)} className="flex items-center gap-2 mb-12 font-headline font-black text-[10px] uppercase tracking-widest text-primary hover:opacity-70 transition-opacity">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-8 mb-12">
            <div className={`flex items-center border-[6px] h-24 px-8 w-full sm:w-auto transition-all duration-700 ${isDark ? 'border-zinc-800 bg-zinc-900/30' : 'border-zinc-950 bg-white'}`}>
              <button onClick={() => setQty(Math.max(1, qty - 1))} className="hover:text-primary transition-all p-2 active:scale-90"><Minus size={24} strokeWidth={3}/></button>
              <span className="w-20 text-center font-headline font-black text-3xl">{qty}</span>
              <button onClick={() => setQty(qty + 1)} className="hover:text-primary transition-all p-2 active:scale-90"><Plus size={24} strokeWidth={3}/></button>
            </div>
            
            <button 
              onClick={handleAddToCart}
              className="w-full sm:flex-1 h-24 bg-primary text-on-primary font-headline font-black uppercase tracking-[0.6em] text-sm hover:translate-y-[-4px] hover:shadow-[0_20px_40px_-15px_rgba(184,207,136,0.6)] active:translate-y-[2px] transition-all shadow-2xl flex items-center justify-center gap-3 italic overflow-hidden relative group"
            >
